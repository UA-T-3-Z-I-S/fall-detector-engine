...
# Global para saber si se presionó siguiente video
...
def mostrar_prediccion_en_video(frames, predicciones, probabilidades=None, video_path=None, caida_detectada=False):
    total_frames = len(frames)
    total_buffers = len(predicciones)
    buffer_size = total_frames // total_buffers if total_buffers > 0 else 1
    caida_count = sum(predicciones)
    ...
    while True:
        if not paused and current_frame < total_frames:
            ...
            buffer_idx = min(current_frame // buffer_size, total_buffers - 1)
            label = predicciones[buffer_idx]
        ...
        if video_control['accion']:
            ...
            if action == 'repetir':
                ...
                ejecutar_prediccion_en_vivo()  # Reinicia el flujo
                return
            elif action == 'salir':
                break
...
def ejecutar_prediccion_en_vivo():
    ...
    start_time = time.time()  # ⏱️ Inicio de procesamiento
    ...
    end_time = time.time()  # ⏱️ Fin de procesamiento
    duracion = end_time - start_time
    print(f"[⏱] Tiempo total de procesamiento: {duracion:.2f} segundos")

    print(f"[✔] Voto final: {result['positivos']} de {result['buffers_totales']} buffers indican caida "
